...
- Install cluster for kubevirt with shared /var/run/kubevirt
  : k3d cluster create --volume /var/run/kubevirt:/var/run/kubevirt:shared cluster1
...
- Connect to virtual machine over VNC
  : sudo KUBECONFIG=/home/oleg/kubeconfig.yaml nsenter -t $(docker inspect --format '{{.State.Pid}}' k3d-cluster1-server-0) -n virtctl vnc vm-guix-datavolume

...
По мере того, как вы пользуетесь операционными системы на базе GNU/Linux, вы
изменяете настройки программ под себя.

Программа хранит свои настройки там, где решил разработчик.

у вас формируется большое количество файлов с настройками программ.
...
и копируем из примера playbook, задачей которой является установка лучшего
...
В завимости от разновидности операционной системы GNU/Linux, следует заменить
**apt** на тот пакетный мэнэджэр, которым вы установили Ansible.
    
Быстрые команды
===============

(англ. Ad-Hoc Commands)

По правде говоря, сила Ansible в playbook, но если вам нужно выключить все
компьютеры перед сном, то можно воспользоваться этим вариантом.

Для начала давайте настроим ssh-agent, чтобы он запоминал пароли.

    $ ssh-agent bash
    $ ssh-add ~/.ssh/id_rsa

Выключить все компьютеры в группе workstations параллельно.

    $ ansible workstations -a "/sbin/poweroff" -f 10 -b
    
Синтаксис
=========

Для тех, кто готов сразу в бой, синтаксис у Ansible такой:

    $ ansible <HOSTNAME> -m <MODULE> 
    
    HOSTNAME может быть hostname или IP адресом удаленной машины, а также localhost
...
Протестировать соединение с удаленной машиной
=============================================

    $ ansible <HOSTNAME> -m ping

* попытаться соединиться с удаленной машиной
* проверить доступность и пригодность python
* вернуть pong
...
**Система управления конфигурациями** - это программа, которая помогает вам
настроить операционную систему, программу, сервис.

Используя управление конфигурацией, вы видите, что у вас было, в какой момент, и
что сейчас находится на компьютере. Вы настраиваете программы в одном месте.
...
Они включают в себя настройку серверной инфаструктуры, причем это не
обязательно какая-то инфраструктура из серверов, на которую нужно зайти и
...
Это например Amazon или Google Cloud, на которых, перед тем как собственно
настраивать сервера, вам сначала нужно их создать.
...
Это довольно очевидно и не всегда просто.
...
Какие решения можно придумать в лоб для решения предыдущих задач?
...
Что естественно видеть к большому количеству проблем.
...
то делать синхронные изменения и контролировать, то что они выполняются
...
Естественно в таком случае поднять новый сервер становится тривиальной задачей
...
А во вторых у вас полноценный turring полной язык, что приводит к совершенно не поддерживаемому коду очень часто.
...
никаких дополнительных файлов, никакой дополнительной предустановки, которая
требовалась бы с более сложными системами.
...
Потому что настройка собственной локальной среды - это уже проблема, которую
можно решить полностью и целиком с помощью ansible.
...
Тесты, проверяющие то как вы выполняете задачи с помощью ansible, написанные на самом ansible.

И это тоже здорово, потому что ansible хорошо подходит для того, чтобы
верифицировать то что делается им самим же.

Его модули легко позволяют проверять текущее состояние системы.

С одной стороны очень удобно, а с другой стороны есть определенная особенность в
том, что запуская тесты, вы будете видеть ansible.
...
Установить пакет
    ...
    ansible all -i hosts -m apt -a "name=vim state=present" -b

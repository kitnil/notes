...
oleg@guixsd ~/src/gitlab.intr/nixos/kubernetes$ kubectl delete ValidatingWebhookConfiguration virt-operator-validator
validatingwebhookconfiguration.admissionregistration.k8s.io "virt-operator-validator" deleted

oleg@guixsd ~/src/gitlab.intr/nixos/kubernetes$ kubectl -n kubevirt patch kv kubevirt --type=merge -p '{"metadata":{"finalizers":[]}}'

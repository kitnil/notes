...
* TODO Don't create /tmp/env-vars on jenkins.intr
  SCHEDULED: <2020-04-01 Wed>
#+begin_example
  [jenkins@nixos:~]$ (set -x; sudo rm -f /tmp/env-vars; ls -l /tmp/env-vars; time nix-build '<nixpkgs>' --no-out-link -A hello --check -K 2>/dev/null; ls -l /tmp/env-vars)
  + sudo rm -f /tmp/env-vars
  + ls --color=tty -l /tmp/env-vars
  ls: cannot access '/tmp/env-vars': No such file or directory
  + nix-build '<nixpkgs>' --no-out-link -A hello --check -K
  ...
  ¯\_(ツ)_/¯
  11:43
  --option sandbox false тоже НЕ провоцирует создание /tmp/env-vars (edited) 
  11:43
  nix-shell -p hello тоже
  --expr 'with import <nixpkgs> {}; pkgs.hello.overrideAttrs(old: { unpackPhase = "false"; } )' тоже
#+end_example
https://github.com/NixOS/nix/issues/262 -> https://github.com/NixOS/nix/issues/920

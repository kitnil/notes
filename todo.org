...
Шиншинова Ксения  12:43
Коллеги, напоминаю, что
24 июня – День парада (среда) – выходной день.
25 июня (четверг) – рабочий день.
Коллеги, работающие по графику, сверяемся с ним
...
: select * from equip_servers where name not like "mj%" and name not like "nh%" order by name;

* TODO

  subscription on sieve directory via imap kills dovecot
...
(ping -DO 172.16.100.60 | grep --invert-match --line-buffered '64 bytes from' |& while read -r line; do notify-send "172.16.100.60: $line"; done) &

...
- [[https://github.com/moasq/production-saas-starter][moasq/production-saas-starter: Production-ready B2B SaaS Starter Kit (Go + Next.js). Modular Monolith. Hexagonal Arch.]]
...
- [[https://github.com/go-monk/from-bash-to-go][go-monk/from-bash-to-go: Bash is great until it isn't]]
...
- [[https://github.com/goforj/godump][goforj/godump: A minimal, developer-friendly pretty-printer and debug dumper for Go structs, inspired by Laravel’s dump() and Symfony’s VarDumper.]]
...
* Programms
** Misc
- [[https://github.com/0xjuanma/golazo][0xjuanma/golazo: The beautiful game in your terminal. Minimal TUI app to keep up with live football/soccer written in Go.]]
...
- [[https://github.com/cvhariharan/flowctl][cvhariharan/flowctl: An open source self-service workflow execution platform]]
...
- [[https://github.com/sysadminsmedia/homebox][sysadminsmedia/homebox: A continuation of HomeBox the inventory and organization system built for the Home User]]
...
- [[https://github.com/ozansz/xgotop][ozansz/xgotop: Realtime Go Runtime Visualization]]
...
Best Practices
    ...
    "Do not communicate by sharing memory; instead, share memory by communicating"
        ...
        Use channels rather than shared variables with mutexes when possible

    Close channels from the sender only

        Prevents panic from closing already closed channels
...
Error Handling in Concurrent Code
    ...
    Use channels for error propagation:
...
go func() { errCh <- doWork() }()
    ...
    Use errgroup package:

go

g, ctx := errgroup.WithContext(ctx)
g.Go(func() error { return doTask1() })
g.Go(func() error { return doTask2() })
err := g.Wait()
...
Common Pitfalls
    ...
    Channel deadlocks (unbuffered channels without receiver)

    Goroutine leaks

    Copying sync types (mutex, waitgroup)

Advanced Topics

    sync.Pool for object reuse

    atomic package for low-level operations

    runtime.GOMAXPROCS() for CPU core configuration

    select with default for non-blocking operations

Go's concurrency model is based on CSP (Communicating Sequential Processes) and provides simple, powerful primitives for writing concurrent programs that are easier to reason about than traditional threading approaches.
